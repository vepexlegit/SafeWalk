package de.vepexlegit.safewalkdebug.mixin.mixins;
...
public abstract class MixinEntity {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = SafeWalkDebug.safewalk && SafeWalk.INSTANCE.isEnabled() ? this.onGround && entity instanceof EntityPlayer : this.onGround && this.isSneaking() && entity instanceof EntityPlayer;
            if (flag) {
                double d6;
                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(entity, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty(); d3 = x) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
                ...
                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(entity, this.getEntityBoundingBox().offset(0.0D, -1.0D, z)).isEmpty(); d5 = z) {
                    if (z < d6 && z >= -d6) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= d6;
                ...
                for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(entity, this.getEntityBoundingBox().offset(x, -1.0D, z)).isEmpty(); d5 = z) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
            ...
            List<AxisAlignedBB> list1 = this.worldObj.getCollidingBoundingBoxes(entity, this.getEntityBoundingBox().addCoord(x, y, z));
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(entity, this.getEntityBoundingBox().addCoord(d3, y, d5));
            ...
            if (d4 != y) {
                block1.onLanded(this.worldObj, entity);
            }
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                if (block1 != null && this.onGround) {
                    block1.onEntityCollidedWithBlock(this.worldObj, blockpos, entity);
                }
